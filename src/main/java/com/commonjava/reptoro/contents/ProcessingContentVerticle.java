...
//                                            .setHandler(this::handleHeaders)
                ...
                if(res.succeeded()) {
                    logger.info("GETTING ALL HEADERS SUCCEEDED");
//                    CompositeFuture.all(
//                        contents.stream()
//                                .map(repoObj -> new JsonObject(repoObj.toString()))
//                                .map(content -> updateContentWithHeaders(content).setHandler(this::handleUpdateHeaders))
//                                .collect(Collectors.toList())
//                    ).onComplete(update -> {
//                        if(update.succeeded()) {
//                            logger.info("===>  ALL SOURCE/LOCAL CONTENT HEADERS UPDATED IN DB");
...
//                        } else {
//                            logger.info("===>  ALL SOURCE/LOCAL CONTENT HEADERS FAILED UPDATING: " + update.cause());
//                        }
//                    });
                } else {
...
//                logger.info("HEADERS FOR REPO: " + asyncResult.result().getString("filesystem") + "\nSTORAGE: " + asyncResult.result().getString("filestorage"));
    ...
    private Future<JsonObject> fetchLocalHeaders(JsonObject content) {
        ...
        contentProcessingService.getLocalHeadersSync(content , res -> {
            if(res.succeeded()) {
                if(!res.result().containsKey("badresponse")) {
...
//            });
    ...
    private Boolean isExceptedFilenameExtension(JsonObject cont) {
        ...
        String filename = cont.getString("filename");
        for(Object ext : exceptedFileExtensions) {
            return String.valueOf(ext).equalsIgnoreCase(filename) || filename.endsWith(String.valueOf(ext));
            // example: "maven-metadata.xml" -> true |  "primername.sha1" -> true
        }
        return false;
    }
